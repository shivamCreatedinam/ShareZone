...
function handleEvent(_ref) {
  ...
  let _data;
  try {
    _data = _jsonBigint.default.parse(data) ?? {};
  } catch (e) {
    _data = {};
  }
  let _event = event;
...
function callIrisApi(funcName, params) {
  try {
      ...
      switch (funcName) {
        ...
        case 'RtcEngine_release':
          _specs.default.callApi({
            funcName,
            params: _jsonBigint.default.stringify(params),
            buffers
          });
    ...
    if (ret !== undefined && ret !== null && ret !== '' && ret !== 'null') {
      const retObj = _jsonBigint.default.parse(ret);
      if (isDebuggable()) {
        if (typeof retObj.result === 'number' && retObj.result < 0) {
          console.error('callApi', funcName, _jsonBigint.default.stringify(params), ret);
        } else {
          console.debug('callApi', funcName, _jsonBigint.default.stringify(params), ret);
        }
      }
      return retObj;
    }
    ...
    if (isDebuggable()) {
      console.error('callApi', funcName, _jsonBigint.default.stringify(params), e);
    } else {
      console.warn('callApi', funcName, _jsonBigint.default.stringify(params), e);
    }
  }
  return {};
}
